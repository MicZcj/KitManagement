...
public class KitShowByDownload extends HttpServlet {
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		if(select.equals("1")){
			PageBean<Tool> pageBean = kitDao.findbyNameDesc(keyword, currPage);
			...
			PageBean<Tool> pageBean = kitDao.findByDescriptionDesc(keyword, currPage);
			request.setAttribute("pageBean", pageBean.getList());
			request.setAttribute("page", pageBean);
			request.getSession().setAttribute("select", select);
			request.getSession().setAttribute("keyword", keyword);
			...
			PageBean<Tool> pageBean = kitDao.findByUserDesc(keyword, currPage);
			request.setAttribute("pageBean", pageBean.getList());
			request.setAttribute("page", pageBean);
			request.getSession().setAttribute("select", select);
			request.getSession().setAttribute("keyword", keyword);
        	...
        	PageBean<Tool> pageBean = kitDao.findByTagDesc(keyword, currPage);
			request.setAttribute("pageBean", pageBean.getList());
			request.setAttribute("page", pageBean);
			request.getSession().setAttribute("select", select);
			request.getSession().setAttribute("keyword", keyword);
